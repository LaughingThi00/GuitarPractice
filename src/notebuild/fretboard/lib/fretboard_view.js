import * as musicKit from "./../../music-model-kit";
import Point from "./point.js";
import log from "./../../log";

export default class FretboardView {
  constructor(
    id = "fretboard_view_id",
    width = 1000,
    onClick,
    hover = false,
    showLabels = true,
  ...
  buildCanvases() {
    this.canvas_background = this.buildCanvas(
      "fretboard_background_canvas_" + this.id,
      this.WIDTH,
      this.HEIGHT
    );
    this.canvas = this.buildCanvas(
      "fretboard_canvas_" + this.id,
      this.WIDTH,
      this.HEIGHT
    );
    if (this.hover) {
      this.canvas_hover = this.buildCanvas(
        "fretboard_canvas_hover_" + this.id,
        this.WIDTH,
        this.HEIGHT
      );
    }
  ...
  draw_background() {
    ...
    var rightMargin = Math.round(this.WIDTH * 0.01);
    ...
    for (i = 1; i <= number_of_frets; i++) {
      ...
      var x = Math.round(
        i * ((this.WIDTH - (rightMargin + leftMargin)) / number_of_frets) +
          buffer
      );
    ...
    var note_x_positions = [
      this.radius + 2,
      (this.WIDTH - (leftMargin + rightMargin)) * 0.078,
      ...
      (this.WIDTH - (leftMargin + rightMargin)) * 0.55, // 9
    ...
    var b_string_y_position =
      (this.HEIGHT - (topMargin + bottomMargin)) * 0.2 + topMargin;
    var g_string_y_position =
      (this.HEIGHT - (topMargin + bottomMargin)) * 0.4 + topMargin;
    var d_string_y_position =
      (this.HEIGHT - (topMargin + bottomMargin)) * 0.6 + topMargin;
    var a_string_y_position =
      (this.HEIGHT - (topMargin + bottomMargin)) * 0.8 + topMargin;
    var e_string_y_position =
      (this.HEIGHT - (topMargin + bottomMargin)) * 1.0 + topMargin;
    var note_y_positions = [
      high_e_string_y_position,
      b_string_y_position,
      g_string_y_position,
      d_string_y_position,
      a_string_y_position,
    ...
    function draw_single_dot_fret_markers(ctx, fret) {
      ...
      ctx.lineWidth = 0;
      ctx.arc(
        note_x_positions[fret],
        (g_string_y_position + d_string_y_position) / 2,
        diameter,
        0,
        TWO_PI
      );
    ...
    function draw_double_dot_fret_markers(fret) {
      ...
      ctx.arc(
        note_x_positions[fret],
        (d_string_y_position + a_string_y_position) / 2,
        diameter,
        0,
        TWO_PI
      );
      ...
      ctx.arc(
        note_x_positions[fret],
        (g_string_y_position + b_string_y_position) / 2,
        diameter,
        0,
        TWO_PI
      );
      ...
      ctx.lineWidth = 0;
      ctx.arc(
        note_x_positions[fret] - seperation,
        HEIGHT - bottomMargin * 0.25,
        diameter,
        0,
        TWO_PI
    ...
    for (j = 0; j < number_of_string; j++) {
      ...
      var y =
        j *
        ((this.HEIGHT - (bottomMargin + topMargin)) / (number_of_string - 1));
  ...
  clearNote(note) {
    ...
    for (i = 0; i < note_positions.length; i++) {
      ctx.clearRect(
        note_positions[i][0] - this.radius - 1,
        note_positions[i][1] - this.radius - 1,
        this.radius * 2 + 2,
        this.radius * 2 + 2
      );
    }
  ...
  drawNote(note) {
    ...
    let canvas = this.canvas;
    var ctx = canvas.getContext("2d");
    var label = this.show_labels
      ? note.note_name.type.substring(0, 2)
      : undefined;
  ...
  drawNotePlaceholder(note, label) {
    this.drawNoteWithColor(
      note,
      label,
      this.darkMode ? "#aaa" : "#ddd",
      this.canvas
    );
  }
    ...
    for (i = 0; i < note_positions.length; i++) {
      ...
      if (label !== undefined) {
        ...
        ctx.fillText(
          label,
          note_positions[i][0],
          note_positions[i][1] + this.radius * 0.3,
          this.radius * 2
        );
      }
    }
  ...
  drawInterval(interval) {
    ...
    var first_note =
      play_type === musicKit.Interval.PLAY_TYPE.ASCENDING
        ? interval.lower_note
        : higher_note;
    ...
    setTimeout(
      ...
      interval.play_type == musicKit.Interval.PLAY_TYPE.HARMONIC
        ? 0
        : interval.delay_in_ms
    );
  }
  ...
  drawChord(chord) {
    ...
    let note_array = chord.getNoteArray(
      musicKit.all_notes,
      musicKit.guitar_range
    );
  ...
  drawScale(scale) {
    ...
    let note_array = scale.getNoteArray(
      musicKit.all_notes,
      musicKit.guitar_range
    );
    var j;
  ...
  addHoverEventListeners() {
    ...
    this.mouseMoveFunction = function (event) {
      ...
      for (const [key, value] of Object.entries(
        self.noteValueToNotePositionsDict
      )) {
        let midi_value = key;
        for (let i = 0; i < value.length; i++) {
          let note_positions = value[i];
