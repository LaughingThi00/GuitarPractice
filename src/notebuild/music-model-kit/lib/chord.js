...
export default class Chord {
	...
	getLabels() {
		...
		for(i=0; i<this.note_sequence.length; i++){
			let sequence = this.note_sequence[i]
			if(sequence < 0) {
	...
	static generateRandom(all_notes, range, types = this.ALL_TYPES, play_types = this.ALL_PLAY_TYPES,
		...
		let random_note = all_notes[randomInteger(min, max)];
		let play_type = play_types[ randomInteger(0, play_types.length-1) ];
		let random_chord_type = types[ randomInteger(0, types.length-1) ];

		
		let inversion = Chord.INVERSION_TYPE.Root;
		...
		let chord = new Chord(random_note, random_chord_type, play_type, inversion);
		let note_array = chord.getNoteArray(all_notes, range)
